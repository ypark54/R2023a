 ...
 *
 *   Alternatively for ERT targets, you can generate a sample ert_main.cu file 
 *   with the generated code by selecting the "Generate an example main program"
 *   option.  In this case, ert_main.cu is precisely customized to the
 *   model requirements.  
 *
 * Required Defines:
 *
...
# error "Classic call interface is not supported by rt_main.cu."
#endif

#if ONESTEPFCN==0
#error Separate output and update functions are not supported by rt_main.cu. \
You must update rt_main.cu  to suit your application needs, or select \
the 'Single output/update function' option.
#endif

#if TERMFCN==0
#error The terminate function is required by rt_main.cu. \
Select model configuration parameter 'Terminate function required' \
or 'Generate an example main program' or modify rt_main.cu to \
meet your application requirements.
#endif

#if MULTI_INSTANCE_CODE==1
#error The static version of rt_main.cu does not support reusable \
code generation.  Either deselect ERT option 'Generate reusable code', \
select option 'Generate an example main program', or modify rt_main.cu for \
...
#error The static version of rt_main.cu does not support model step function prototype control.
#endif
