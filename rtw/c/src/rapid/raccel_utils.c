 ...
 */
void
rt_UpdateParamInfoForStringDataType(
    const mxArray *mat,
    ParamInfo *paramInfo)
{
 ...
 */
bool
rt_IsStringData(const mxArray *mat)
{
    if (!mxIsCell(mat))
    {
        return false;
...
extern const char* rt_CreateFullPathToTop(const char* toppath, const char* subpath)
{
    ...
    fullpath = (char*)malloc((strlen(toppath) + strlen(subpath) + 1) * sizeof(char));
...
const char* rt_RAccelReadInportsMatFile(SimStruct* S,
    ...
    *matFileFormat = fileDataFormat; 
    ...
    for (inportIdx = 0; inportIdx < gblNumRootInportBlks; ++inportIdx) {
        ...
        slsaSaveRawMemoryForSimTargetOP(S, currTimeIdxKey,
                (void**)(&(gblInportTUtables[inportIdx].currTimeIdx)),
                sizeof(gblInportTUtables[inportIdx].currTimeIdx), NULL, NULL);
    }
