...
{     
    ...
    if (numDimsValues == 2){
        if (dimsValues[1] != (size_t)gblInportDims[inportIdx*2]){
    ...
    if (numDimsValues == 3){
      if (dimsValues[0] != (size_t)gblInportDims[inportIdx*2] 
            ||dimsValues[1] != (size_t)gblInportDims[inportIdx*2+1]){
            (void)sprintf(errmsg,"Dimension of signal #%d from MAT-file not " 
                          "equal the dimension of the corresponding inports "
...
static const char* setGblInportTUtableElement(int_T inportIdx,
     ...
     * way. See rt_RAccelReadInportsMatFile/RSimReadInportsMatFile for details. */
    gblInportTUtables[inportIdx].currTimeIdx = (int*)calloc(1, sizeof(int));
    if (gblInportTUtables[inportIdx].currTimeIdx == NULL) {
    ...
    if (matFileDataFormat == SINGLEVAR_MATRIX) {
        ...
        for (inportIdx = 0; inportIdx < gblNumRootInportBlks; ++inportIdx) {
            ...
            if(isComplex) {
                ...
                size_t nrows = mxGetM(mxInportSignalValues);
                size_t ncols = mxGetN(mxInportSignalValues);
                matDataRe = (char*)calloc(nrows*ncols,halfElementSize);
                matDataIm = (char*)calloc(nrows*ncols,halfElementSize);
                deInterleaveComplex(matDataRe, matDataIm, matData, 
                                    halfElementSize, nrows, ncols);
            } else {
                matDataRe = matData;
            }
    ...
    if(gblNumRootInportBlks>0){
        ...
        if (gblInportTUtables!= NULL){
            for(i=0; i< gblNumRootInportBlks; i++){
                ...
                if(gblInportTUtables[i].currTimeIdx != (int*)NULL){
                    free(gblInportTUtables[i].currTimeIdx);
                    gblInportTUtables[i].currTimeIdx = NULL;
                }
            }
...
// ScopedDSM does not support diagnostics in rapid accel.
void slmrRegisterScopedDSMAccess(SimStruct* S,
