...
LIBMWSF_RUNTIME_API SimStruct* sf_mex_set_simstruct(SimStruct* S);
LIBMWSF_RUNTIME_API SimStruct* sf_mex_get_simstruct(void);
...
LIBMWSF_RUNTIME_API const mxArray* sf_mex_call(emlrtConstCTX aTLS, emlrtMCInfo *aLoc, const char* functionName, unsigned int numReturn, unsigned int numArguments, ...);
...
LIBMWSF_RUNTIME_API mxArray* sf_mex_dup(const mxArray* srcMxArray);
...
LIBMWSF_RUNTIME_API mxArray* sf_mex_duplicatearraysafe(const mxArray* const* ppMxArray);
...
LIBMWSF_RUNTIME_API void sf_mex_lw_impx_size_check_nd(const int* dims1, const int* dims2, int nDims);
...
LIBMWSF_RUNTIME_API  void sf_mex_lw_dim_size_impx_check(int dim1, int dim2, int index);
...
#define EML_FOR_LOOP_SINGLE_NUMEL_ROUNDOFF "Loop range with length %g cannot be represented exactly by index variable of 'single' type. Rounding of index value might produce unexpected results. Use index variable of type double or integer."
...
LIBMWSF_RUNTIME_API void sf_mex_clear_bus_parameter_worklist(void);

LIBMWSF_RUNTIME_API void ssGetSFcnPrimitiveDataStoreNameAddrIdx_wrapper(SimStruct* S,
                                                                        const char* name,
                                                                        void** dsmAddress);
LIBMWSF_RUNTIME_API void ssGetSFcnUnboundedDataStoreNameAddrIdx_wrapper(SimStruct* S,
                                                                        const char* name,
                                                                        void** dsmAddress);
LIBMWSF_RUNTIME_API void ssReadFromDataStore_wrapper(SimStruct* S,
                                                     int dsmIndex,
                                                     const char* dsmName);
...
LIBMWSF_RUNTIME_API void ssWriteToDataStoreAndLogToDI_wrapper(SimStruct* S, int dsmIndex, const char* dsmName);
...
LIBMWSF_RUNTIME_API void ssResizeDSM_wrapper(SimStruct* S, const char* name, int dIdx, int newSize);
